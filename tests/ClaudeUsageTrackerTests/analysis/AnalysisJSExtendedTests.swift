...
final class AnalysisJSExtendedTests: AnalysisJSTestCase {
    ...
    // Cumulative cost and stats tests removed (C1: logic re-implementation).
    // These are covered by AnalysisTemplateRenderTests which calls actual template functions
    // (renderCumulativeTab, renderMain) instead of re-implementing the formulas.
