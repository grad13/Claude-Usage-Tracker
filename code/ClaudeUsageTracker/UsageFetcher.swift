...
enum UsageFetcher {
    ...
    static func hasValidSession(using webView: WKWebView) async -> Bool {
        ...
        NSLog("[ClaudeUsageTracker] hasValidSession: %d cookies total, %d claude.ai", cookies.count, claudeCookies.count)
        let now = Date()
        ...
        if !hasSession {
            ...
            NSLog("[ClaudeUsageTracker] hasValidSession: NO sessionKey found. claude.ai cookies: %@", names)
        }
        return hasSession
    }
